...
# 💡 [복구 에러 해결] 기호로 시작하는 문자열이 수식으로 오인되어 엑셀이 파괴되는 현상 방지
...
def build_advanced_excel_report():
    ...
    # 💡 [원래 모양 복원 핵심] 1단계에서 생성된 파일명 -> 원본 URL 매핑 테이블 사전 로드
    js_url_converter = {}
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    # 💡 [원래 모양 복원 연동 기전] 언더바 파일명을 원래의 예쁜 URL 주소 형태로 치환
    ...
    for domain, url_map in matrix_data.items():
        ...
        for sub_idx, (url, data) in enumerate(sorted(url_map.items()), 1):
            ...
            for c in range(1, 5):
                ...
                if c in [3, 4]:
                    cell.alignment = align_left  # 한 줄 슬라이스 규격 (줄바꿈 해제 상태 보존)
                else:
                    cell.alignment = align_center
    ...
    for sheet in wb.worksheets:
        ...
        for col_idx, col in enumerate(sheet.columns, 1):
            ...
            if header_value in ["High Risk URL / Endpoint", "Target URL / Endpoint"]:
                ...
                sheet.column_dimensions[col_letter].width = 50  # 복원된 온전한 긴 주소 가독성 마진폭 확보
